...
async function fixWebhook() {
    console.log(`--- Configurando Webhook (Formato Alternativo) para: ${INSTANCE_NAME} ---`);
    ...
    try {
        const response = await fetch(`${EVOLUTION_API_URL}/instance/setWebhook/${INSTANCE_NAME}`, {
            method: "POST",
            headers: {
        ...
        console.log(`Status /instance/setWebhook: ${response.status}`);
        console.log(`Response: ${text}`);

        // Tentativa 2: /webhook/set (Formato simplificado)
        ...
        console.log(`Status /webhook/set: ${response2.status}`);
