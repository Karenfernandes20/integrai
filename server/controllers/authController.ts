...
export const login = async (req: Request, res: Response) => {
    try {
        ...
        if (user.company_id) {
            const compRes = await pool.query('SELECT id, name, logo_url, plan_id, due_date, category, operation_type, operational_profile FROM companies WHERE id = $1', [user.company_id]);
