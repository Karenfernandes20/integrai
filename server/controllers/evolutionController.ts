...
export const syncEvolutionConversations = async (req: Request, res: Response) => {
  const config = await getEvolutionConfig((req as any).user, 'syncConversations');
  const EVOLUTION_API_URL = config.url;
  const EVOLUTION_API_KEY = config.apikey;
  const EVOLUTION_INSTANCE = config.instance;
  ...
  try {
    ...
    console.log(`[Evolution] Syncing chats from: ${url}`);
    ...
    if (!response.ok) {
      const text = await response.text();
      console.warn(`[Evolution] Sync chats failed: ${text}`);
      return res.status(response.status).json({ error: "Failed to fetch from Evolution", details: text });
    }

    const data: any[] = await response.json();
    console.log(`[Evolution] Fetched ${data.length} chats. Saving to DB...`);
    ...
    if (pool && Array.isArray(data)) {
      for (const chat of data) {
        ...
        await pool.query(`
                INSERT INTO whatsapp_conversations (external_id, phone, contact_name, unread_count, instance, updated_at)
                VALUES ($1, $2, $3, $4, $5, NOW())
                ON CONFLICT (external_id, instance) 
                DO UPDATE SET 
                    contact_name = EXCLUDED.contact_name,
                    unread_count = EXCLUDED.unread_count,
                    updated_at = NOW();
            `, [jid, phone, name, unreadCount, EVOLUTION_INSTANCE]);
      }
    }
