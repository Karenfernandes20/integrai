...
(async () => {
    try {
        ...
        if (users.rows.length === 0) {
            ...
            const comps = await pool.query("SELECT id, name FROM companies WHERE name ILIKE '%wagdaloja%' OR name ILIKE '%wagda%'");
        ...
        if (instance.rows.length === 0) {
            ...
            if (compLegacy.rows.length === 0) {
                ...
                // Check all instances
                const allInst = await pool.query("SELECT instance_key, name, company_id FROM company_instances LIMIT 10");
                console.log("Instâncias Existentes (Exemplo):", allInst.rows);
            } else {
